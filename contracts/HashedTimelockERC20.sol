...
*
* This contract provides a way to create and keep HTLCs for $ONCE token.
*
*
* Protocol:
*
*  1) newContract(hashlock, timelock, tokenContract, amount) - a
*     sender calls this to create a new HTLC on a given token (tokenContract)
*     for a given amount. A 32 byte contract id is returned
*  3) refund() - after timelock has expired creator of the HTLC can get their tokens
...
contract HashedTimelockERC20 {
    ...
    event HTLCERC20TimeLockExtended(bytes32 indexed contractId);
     ...
     * funds and providing the hashlock.
     *
     * NOTE: Sender must first call approve() on the token contract.
    ...
    /**
     * @dev Called by the sender when the time lock has
     * expired. This will restore ownership of the tokens to the sender.
     *
     * @param _contractId Id of HTLC to refund from.
     * @return bool true on success
     */
    function refund(bytes32 _contractId, bytes32 _preimage)
        external
     ...
     *
     * @param _contractId Id of HTLC to refund from.
     * @param _timelock UNIX epoch seconds new time the lock expires at.
     * @return bool true on success
     */
    function extendTimeLock(bytes32 _contractId, bytes32 _preimage, uint256 _timelock)
        external
        contractExists(_contractId)
        hashlockMatches(_contractId, _preimage)
        futureTimelock(_timelock)
        ...
        returns (bool)
    {
        ...
        emit HTLCERC20TimeLockExtended(_contractId);
        return true;
    }
